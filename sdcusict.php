<?php include 'sdcusict2022header.php' ?>
  <!-- carousel -->
  <?php include 'sdcusict2022carousal.php' ?>

  <div class="container-fluid a" id="op">
    <div class="row">
      <div class="coloumb">
        <p class="caption101"> Official projects </p>
      </div>
    </div>
  </div>

  <!-- As a link -->


  <div class="top line anim-typewriter">
    <p class="fs-1 typ">Software Development</p>
    <p class="fs-1 typ"> Cell USIC&T</p>
  </div>
  <!-- Carousal-2 -->
  <div id="carousel--2" class="carousel slide my-crousel" data-bs-ride="carousel">
    <div class="carousel-inner --carousel-inner---mid-carousel">
      <div class="circle"></div>


      <div class="carousel-item active my-crousel--slide">
        <div class="container container--for-mid-carousel d-flex flex-column align-items-center carousel--2">
          <img src="ic1.png" alt="something" class="imgg" />
          <p class="my-crousel--heading">Student Feedback System team</p>
          <p class="my-crousel--content">
            Student Feedback System(SFS) portals are 
        solely for the purpose of receiving feedback
        from students of all programs for their respective
        faculties allotted during a particular semester.
        This procedure takes place towards the end of each
        semester in all USS's of GGSIPU.
          </p>
        </div>
      </div>

      <div class="carousel-item my-crousel--slide">
        <div class="container container--for-mid-carousel d-flex flex-column align-items-center carousel--2">
          <img src="ic2.png" alt="something" class="imgg" />
          <p class="my-crousel--heading">Web Development Team</p>
          <p class="my-crousel--content">
            Web Development Team works on the
            official websites under the Software
            Development Cell, University School Of
            Information, Communication And Technology.

          </p>
        </div>
      </div>

      <!-- <div class="carousel-item my-crousel--slide">
        <div class="container container--for-mid-carousel d-flex flex-column align-items-center carousel--2">
          <img src="ic3.png" alt="something" class="imgg" />
          <p class="my-crousel--heading">student feedback system team 3</p>
          <p class="my-crousel--content">
            Lorem ipsum dolor sit amet consectetur adipisicing elit.
            Architecto natus, recusandae quaerat aspernatur quam aliquid
            aliquam eligendi excepturi quibusdam sunt sequi consequuntur in.
            Nemo ipsam minima necessitatibus neque deserunt ullam.
          </p>
        </div>
      </div> -->
    </div>
  </div>
  </div>

  <!-- As a link -->

  <!-- <div class="container-fluid a" id="op">
    <div class="row">
      <div class="coloumb">
        <p class="caption101"> Official projects </p>
      </div>
    </div>
  </div> -->



  <!-- Meet Our Team + Core Team -->
  <div class="cards">
    <div class="faculty">
      <div class="ellipse">
        <p class="teamheading">MEET OUR TEAM</p>
      </div>
      <div class="trapezium f">
        <p class="heading-card">FACULTY</p>
      </div>
      <div class="row" style="margin: 2%">
        <div class="col-md-4 col-sm-6" style="margin: auto">
          <div class="our-team">
            <div class="pic">
              <img src="../data0/new-website-images/rahulsir.jpg" />
            </div>
            <h3 class="title">Dr. Rahul Johari</h3>
            <div class="post">Faculty Advisor, SDC USICT</div>
            <ul class="social">
              <li>
                <a href="https://twitter.com/rahuljohaari" target="_blank">
                  <img class="img-c" src="twitter-circle.png">
                </a>
              </li>
              <li>
                <a href="mailto:rahul@ipu.ac.in" target="_blank">
                  <img class="img-c" src="mail-circle.png">
                </a>
              </li>
              <li>
                <a href="https://linkedin.com/in/rahuljohari7" target="_blank">
                  <img class="img-c" src="linkedin-circle.png">
                </a>
              </li>
            </ul>
          </div>
        </div>
        <!-- <div class="col-md-4 col-sm-6" style="margin: auto">
          <div class="our-team">
            <div class="pic">
              <img src="http://www.ipu.ac.in/data0/new-website-images/rahulsir.jpg" />
            </div>
            <h3 class="title">Dr. Rahul Johari</h3>
            <span class="post">Faculty Advisor, SDC USICT</span>
            <ul class="social">
              <li>
                <a href="https://twitter.com/rahuljohaari" target="_blank">
                  <img class="img-c" src="twitter-circle.png">
                </a>
              </li>
              <li>
                <a href="mailto:rahul@ipu.ac.in" target="_blank">
                  <img class="img-c" src="mail-circle.png">
                </a>
              </li>
              <li>
                <a href="https://linkedin.com/in/rahuljohari7" target="_blank">
                  <img class="img-c" src="linkedin-circle.png">
                </a>
              </li>
            </ul>
          </div>
        </div>
        <div class="col-md-4 col-sm-6" style="margin: auto">
          <div class="our-team">
            <div class="pic">
              <img src="http://www.ipu.ac.in/data0/new-website-images/rahulsir.jpg" />
            </div>
            <h3 class="title">Dr. Rahul Johari</h3>
            <span class="post">Faculty Advisor, SDC USICT</span>
            <ul class="social">
              <li>
                <a href="https://twitter.com/rahuljohaari" target="_blank">
                  <img class="img-c" src="twitter-circle.png">
                </a>
              </li>
              <li>
                <a href="mailto:rahul@ipu.ac.in" target="_blank">
                  <img class="img-c" src="mail-circle.png">
                </a>
              </li>
              <li>
                <a href="https://linkedin.com/in/rahuljohari7" target="_blank">
                  <img class="img-c" src="linkedin-circle.png">
                </a>
              </li>
            </ul>
          </div>
        </div> -->
      </div>
    </div>
    <hr class="divider">
    <div class="container-card" id="core-team">
      <div class="trapezium">
        <p class="heading-card" style="margin-top: -47px;">CORE TEAM</p>
      </div>
      <div class="row" style="margin: 2%">
        <div class="col-md-3 col-sm-6">
          <div class="our-team">
            <div class="pic">
              <img src="data0/new-website-images/himesh.jpeg" />
            </div>
            <h3 class="title">Himesh Nayak</h3>
            <span class="post">LEAD, SDC USICT</span>
            <ul class="social">
              <li>
                <a href="https://github.com/HimeshNayak" target="_blank">
                  <img src="github-circle.png" class="img-c">
                </a>
              </li>
              <li>
                <a href="https://linkedin.com/in/himeshnayak015" target="_blank">
                  <img class="img-c" src="linkedin-circle.png">
                </a>
              </li>
              <li>
                <a href="mailto:himeshnayak015@gmail.com" target="blank">
                  <img src="mail-circle.png" class="img-c">
                </a>
              </li>
              <li>
                <a href="https://twitter.com/Himesh_nayak" target="blank">
                  <img src="twitter-circle.png" class="img-c">
                </a>
              </li>
            </ul>
          </div>
        </div>
        <div class="col-md-3 col-sm-6">
          <div class="our-team">
            <div class="pic">
              <img src="data0/new-website-images/shubham.jpg" />
            </div>
            <h3 class="title">Shubham Goyal</h3>
            <span class="post">Fullstack Developer</span>
            <ul class="social">
              <li>
                <a href="https://github.com/ShubhamGoyal7125/" target="_blank">
                  <img src="github-circle.png" class="img-c">
                </a>
              </li>
              <li>
                <a href="https://www.linkedin.com/in/shubham-goyal-7987461b8" target="_blank">
                  <img class="img-c" src="linkedin-circle.png">
                </a>
              </li>
              <li>
                <a href="mailto:shubhamgoyal7125@gmail.com" target="_blank">
                  <img class="img-c" src="mail-circle.png">
                </a>
              </li>
            </ul>
          </div>
        </div>
        <div class="col-md-3 col-sm-6">
          <div class="our-team">
            <div class="pic">
              <img src="data0/new-website-images/jatin.jpg" />
            </div>
            <h3 class="title">Jatin Singh</h3>
            <span class="post">Fullstack Developer</span>
            <ul class="social">
              <li>
                <a href="https://github.com/Jasngh004" target="_blank">
                  <img src="github-circle.png" class="img-c">
                </a>
              </li>
              <li>
                <a href="https://www.linkedin.com/in/jatin-singh04/" target="_blank"><img src="linkedin-circle.png"
                    class="img-c"></a>
              </li>
              <li>
                <a href="mailto:jatin2002singh@gmail.com" target="_blank">
                  <img src="mail-circle.png" class="img-c">
                </a>
              </li>
            </ul>
          </div>
        </div>
        <div class="col-md-3 col-sm-6">
          <div class="our-team">
            <div class="pic">
              <img src="data0/new-website-images/harshit.jpeg" />
            </div>
            <h3 class="title">Harshit Panwar</h3>
            <span class="post">Fullstack Developer</span>
            <ul class="social">
              <li>
                <a href="https://github.com/Harshitp7" target="_blank">
                  <img src="github-circle.png" class="img-c">
                </a>
              </li>
              <li>
                <a href="https://www.linkedin.com/in/harshitpanwar7" target="_blank">
                  <img src="linkedin-circle.png" class="img-c">
                </a>
              </li>
              <li>
                <a href="mailto:panwar.harshit0602@gmail.com" target="_blank">
                  <img class="img-c" src="mail-circle.png">
                </a>
              </li>
            </ul>
          </div>
        </div>
        <div class="col-md-3 col-sm-6">
          <div class="our-team">
            <div class="pic">
              <img src="./data0/new-website-images/anurag.jpeg" />
            </div>
            <h3 class="title">Anurag Parashar</h3>
            <span class="post">Fullstack Developer</span>
            <ul class="social">
              <li>
                <a href="https://github.com/AnuragParashar2000" target="_blank">
                  <img src="github-circle.png" class="img-c">
                </a>
              </li>
              <li>
                <a href="https://www.linkedin.com/in/anurag-parashar/" target="_blank">
                  <img class="img-c" src="linkedin-circle.png">
                </a>
              </li>
              <li>
                <a href="mailto:anuragparashar03@gmail.com" target="_blank">
                  <img class="img-c" src="mail-circle.png">
                </a>
              </li>
            </ul>
          </div>
        </div>
        <div class="col-md-3 col-sm-6">
          <div class="our-team">
            <div class="pic">
              <img src="./data0/new-website-images/toyeesh.jpg" />
            </div>
            <h3 class="title">Toyeesh Sinha</h3>
            <span class="post">Fullstack Developer</span>
            <ul class="social">
              <li>
                <a target="_blank" href="https://github.com/sinha-toyeesh">
                  <img src="github-circle.png" class="img-c">
                </a>
              </li>
              <li>
                <a href="https://www.linkedin.com/in/toyeesh-sinha/" target="_blank">
                  <img class="img-c" src="linkedin-circle.png">
                </a>
              </li>
              <li>
                <a target="_blank" href="mailto:toyeshsainik@gmail.com">
                  <img class="img-c" src="mail-circle.png">
                </a>
              </li>
            </ul>
          </div>
        </div>
        <div class="col-md-3 col-sm-6">
          <div class="our-team">
            <div class="pic">
              <img src="data0/new-website-images/manik.jpg" />
            </div>
            <h3 class="title">Manik Aggarwal</h3>
            <span class="post">Fullstack Developer</span>
            <ul class="social">
              <li>
                <a href="https://github.com/Manik-Aggarwal" target="_blank">
                  <img src="github-circle.png" class="img-c">
                </a>
              </li>
              <li>
                <a href="https://linkedin.com/in/manik-aggarwal-640110188" target="_blank">
                  <img class="img-c" src="linkedin-circle.png">
                </a>
              </li>
              <li>
                <a target="_blank" href="mailto:manikaggarwal993@gmail.com">
                  <img class="img-c" src="mail-circle.png">
                </a>
              </li>
            </ul>
          </div>
        </div>
        <div class="col-md-3 col-sm-6">
          <div class="our-team">
            <div class="pic">
              <img src="data0/new-website-images/saksham.jpg" />
            </div>
            <h3 class="title">Saksham Varma</h3>
            <span class="post">Graphics Designer</span>
            <ul class="social">
              <li>
                <a href="https://github.com/SakshamVarma" target="_blank">
                  <img src="github-circle.png" class="img-c">
                </a>
              </li>
              <li>
                <a target="_blank" href="https://www.linkedin.com/in/saksham-varma-45a218209"><img
                    src="linkedin-circle.png" class="img-c"></a>
              </li>
              <li>
                <a target="_blank" href="mailto:saksham.varma02@gmail.com">
                  <img class="img-c" src="mail-circle.png">
                </a>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </div>
    <div class="container-card">
      <div class="trapezium">
        <p>2013-2017 Batch</p>
      </div>
      <div class="row" style="margin: 2%">
        <div class="col-md-3 col-sm-6">
          <div class="our-team">
            <div class="pic">
              <img src="./data0/new-website-images/shubhamsr.jpg" />
            </div>
            <h3 class="title">Shubham Agrawal</h3>
            <span class="post">Fullstack Developer</span>
            <ul class="social">
              <li>
                <a href="https://www.linkedin.com/in/shubham641" target="_blank">
                  <img class="img-c" src="linkedin-circle.png">
                </a>
              </li>
            </ul>
          </div>
        </div>
        <div class="col-md-3 col-sm-6">
          <div class="our-team">
            <div class="pic">
              <img src="./data0/new-website-images/prateek.jpg" />
            </div>
            <h3 class="title">Prateek Jain</h3>
            <span class="post">Fullstack Developer</span>
            <ul class="social">
              <li>
                <a href="https://www.linkedin.com/in/prateek-jain-a8830b114" target="_blank">
                  <img class="img-c" src="linkedin-circle.png">
                </a>
              </li>
            </ul>
          </div>
        </div>
        <div class="col-md-3 col-sm-6">
          <div class="our-team">
            <div class="pic">
              <img src="data0/new-website-images/piyush.jpg" />
            </div>
            <h3 class="title">Piyush Bhandula</h3>
            <span class="post">Fullstack Developer</span>
            <ul class="social">
              <li>
                <a href="https://www.linkedin.com/in/piyushbhandula" target="_blank">
                  <img class="img-c" src="linkedin-circle.png">
                </a>
              </li>
            </ul>
          </div>
        </div>
        <div class="col-md-3 col-sm-6">
          <div class="our-team">
            <div class="pic">
              <img src="data0/new-website-images/himank.jpg" />
            </div>
            <h3 class="title">Himank Bhalla</h3>
            <span class="post">Fullstack Developer</span>
            <ul class="social">
              <li>
                <a href="https://www.linkedin.com/in/himank-bhalla" target="_blank">
                  <img class="img-c" src="linkedin-circle.png">
                </a>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </div>
  </div>

  <?php include 'sdcusict2022footer.php' ?>
